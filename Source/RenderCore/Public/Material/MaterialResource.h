...
namespace rendercore
{
	...
	private:
		void CreateGraphicsResource( const ShaderBase* (&shaders)[agl::MAX_SHADER_TYPE<uint32>] );
		void BindResource( const ShaderBase* shader, agl::SingleShaderBindings& binding );

		void UpdateToGPU( const ShaderBase* (&shaders)[agl::MAX_SHADER_TYPE<uint32>], const NamedShaderParameterList& parametersToUpdate );
