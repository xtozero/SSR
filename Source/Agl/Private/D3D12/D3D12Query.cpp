...
namespace agl
{
	...
	void D3D12OcclusionTest::InitResource()
	{
		...
		[[maybe_unused]] HRESULT hr = D3D12Device().CreateFence( 0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS( &m_completionFence ) );
		assert( SUCCEEDED( hr ) );
	}
	...
	{
		bool bSupportsMeshShader = GetInterface<IAgl>()->SupportsMeshShader();
