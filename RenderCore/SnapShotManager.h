...
public:
	...
	std::shared_ptr<ITexture> CreateCloneTexture( ID3D11Device* pDevice, std::shared_ptr<ITexture>& pSourceTexture, const String& textureName );
	bool TryCreateShaderResource( ID3D11Device* pDevice, std::shared_ptr<ITexture>& pTexture, const D3D11_TEXTURE2D_DESC& desc, const String& textureName, int srcFlag = 0 );
