...
public:
	...
	virtual bool RegisterTexture2D( const String& textureName, void* pTexture ) override;
	...
	// RenderTarget
	virtual IRenderResource* CreateRenderTarget( const ITexture* pTexture, const String& renderTargetName, const TEXTURE_TRAIT* trait = nullptr ) override;
	virtual IRenderResource* CreateDepthStencil( const ITexture* pTexture, const String& depthStencilName, const TEXTURE_TRAIT* trait = nullptr ) override;
	...
	virtual IRenderResource* CreateShaderResource( const ITexture* pTexture, const String& resourceName, const TEXTURE_TRAIT* trait = nullptr ) override;

	void RegisterShaderResource( const String& resourceName, Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>& srView );

	// UTIL
	virtual ITexture* CreateCloneTexture( const ITexture* pSourceTexture, const String& textureName ) override;
	void TakeSnapshot( ID3D11DeviceContext* pDeviceContext, const String& sourceTextureName, const String& destTextureName );

	CD3D11ResourceManager( ID3D11Device* pDevice );
