...
namespace
{
	...
	void BlendDescHandler( CD3D11BlendStateFactory* owner, const String&, const KeyValue* keyValue )
	{
...
public:
	...
	CBlendState( ID3D11DeviceContext* pDeviceContext ) : m_pDeviceContext( pDeviceContext ) { assert( m_pDeviceContext ); }
private:
...
void CBlendState::Set( const SHADER_TYPE type )
{
	...
	m_pDeviceContext->OMSetBlendState( m_pBlendState.Get( ), m_blendFactor.data(), m_sampleMask );
}
...
public:
	...
	CNullBlendState( ID3D11DeviceContext* pDeviceContext ) : m_pDeviceContext( pDeviceContext ) { assert( m_pDeviceContext ); }
private:
...
IRenderState* CD3D11BlendStateFactory::GetBlendState( ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, const String& stateName )
{
	...
	if ( pDevice )
	{
		...
		{
			std::unique_ptr<CBlendState> newState = std::make_unique<CBlendState>( pDeviceContext );
	...
	std::unique_ptr<CNullBlendState> nullState = std::make_unique<CNullBlendState>( pDeviceContext );
	CNullBlendState* ret = nullState.get( );
	m_blendState.emplace( _T( "NULL" ), std::move( nullState ) );
	return ret;
}

void CD3D11BlendStateFactory::AddBlendDesc( const String & descName, const CD3D_BLEND_DESC& newDesc )
{
	m_blendStateDesc.emplace( descName, newDesc );
}

CD3D11BlendStateFactory::CD3D11BlendStateFactory( )
{
	RegisterHandler( RASTERIZER_DESC_HANDLER_KEY_NAME, BlendDescHandler );
}

void CD3D11BlendStateFactory::LoadRasterizerDesc( const KeyValueGroup* pKeyValues )
{
